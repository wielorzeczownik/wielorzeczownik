...
def test_split_languages_keeps_markup_out_of_programming() -> None:
    programming, markup = split_languages(LANG_BYTES)
...
def test_split_languages_honours_the_limits() -> None:
    programming, markup = split_languages(
        LANG_BYTES, programming_limit=2, markup_limit=1
    )
...
def test_split_languages_picks_the_biggest_before_sorting_by_name() -> None:
    ...
    by_size, _ = split_languages(LANG_BYTES, programming_limit=2)
    by_name, _ = split_languages(LANG_BYTES, programming_limit=2, sort="name")
...
def test_usage_segments_return_nothing_when_there_is_no_bar_to_draw(
    ...
    assert usage_segments(lang_bytes, cells, top) == []
...
def test_usage_segments_name_sort_keeps_percentages_with_languages() -> None:
    ...
    by_size = {
        ...
        for segment in usage_segments(LANG_BYTES, 40)
    }
    by_name = {
        segment.name: segment.percent
        for segment in usage_segments(LANG_BYTES, 40, sort="name")
    }
